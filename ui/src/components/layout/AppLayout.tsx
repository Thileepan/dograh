...
function AppHeader() {
  ...
  // Mobile-only: hamburger + wordmark. On md+ every remaining control lives
  // in the sidebar, so the bar is hidden entirely.
  return (
    <header className="sticky top-0 z-50 flex items-center justify-between border-b border-border/60 bg-background/70 px-4 py-2 backdrop-blur-md supports-[backdrop-filter]:bg-background/55 md:hidden">
      <div className="flex items-center gap-3">
        ...
        <Link href="/" className="text-lg font-bold">DCXworks</Link>
      </div>
    </header>
  );
}
