...
async def _get_from_numbers_count(
    organization_id: int, telephony_configuration_id: Optional[int] = None
) -> int:
    """Active phone-number count for the given telephony config (org-scoped),
    falling back to the org's default config when none is provided.
...
async def _validate_max_concurrency(
    ...
    """Validate max_concurrency against org limit and the phone numbers of the
    campaign's telephony configuration (default config when none is pinned).
...
async def create_campaign(
    ...
    # Validate concurrency against the resolved config's caller-id supply —
    # the dispatcher pools CLIs from this config, not the org default.
